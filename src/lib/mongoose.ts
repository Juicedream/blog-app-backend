...
export const connectToDatabase = async (): Promise<void> => {
  ...
  try {
    ...
    logger.info('Connected to the Blog Db successfully', {
...
/**
 * Disconnecrs from the MongoDB database using Mongoose.
 * 
...
export const disconnectFromDatabase = async (): Promise<void> => {
    try {
        await mongoose.disconnect();
         logger.info('Disconnected from the Blog Db successfully', {
        uri: config.MONGO_URI,
        options: clientOptions
    })
    } catch (err) {
